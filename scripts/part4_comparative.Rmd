...
```{r distance-plot, fig.width=10, fig.height=5.8}
p_distance <- ggplot(
  distance_violin,
...
```{r top-bus-destinations, fig.width=12, fig.height=9}
...
```{r top-mrt-destinations, fig.width=12, fig.height=8}
...
```{r catchment-plot, fig.width=10, fig.height=5.8}
ring_colors <- c(
  "0-5 km" = "#D9F1EF",
  "5-10 km" = "#B8E1E3",
  "10-15 km" = "#7BC3D4",
...
```{r heatmap-plot, eval=FALSE, include=FALSE}
p_heatmap <- ggplot() +
  geom_sf(
    data = sg_subzones,
    fill = "#FAFAF7",
  ...
  stat_summary_2d(
    data = heatmap_data,
    aes(x = dest_lon, y = dest_lat, z = TOTAL_TRIPS),
    fun = sum,
    bins = 42,
    alpha = 1
  ) +
  scale_fill_gradientn(
    colours = c("#FFF1D0", "#FDB366", "#F46D43", "#C81D25"),
    trans = "log10",
    labels = comma,
    name = "Trips",
    na.value = NA
  ...
  facet_wrap(~ origin_university, ncol = 2) +
  labs(
    title = "Destination Hotspots Across Singapore",
    subtitle = "Heat intensity represents summed trip volume by destination location over the full study period",
    x = NULL,
    y = NULL
  ) +
  theme_void() +
  theme(
    ...
    panel.border = element_rect(colour = "grey82", fill = NA, linewidth = 0.35)
  )

print(p_heatmap)
ggsave(
  file.path(figures_dir, "part4_fig7_destination_heatmap.png"),
  p_heatmap,
  width = 10.5,
  height = 8.5,
  dpi = 300
...
```{r density-map, fig.width=10.5, fig.height=8.5}
...
density_map_colors <- univ_colors  # Standardised to Part 4 palette
)
...
```{r topic-share-bar, fig.width=10, fig.height=5.8}
...
```{r sankey-prep, message=FALSE, warning=FALSE}
sf_use_s2(FALSE)
...
```{r sankey-plot, fig.width=12, fig.height=9}
sankey_colors <- univ_colors  # Standardised to Part 4 palette
)
...
## Station-Level Topic Decomposition

For the over-indexing university–topic pairs identified above, decompose
*which specific destination stations* drive each association. This answers
the interpretive questions needed for the paper's Results §3.5.

```{r station-decomposition-helper}
#' Compute station-level trip contribution table for a university–topic pair.
#'
#' @param uni   University name (character): "NUS", "NTU", "SMU", or "SUTD"
#' @param topic Topic label (character): must match topic_levels
#' @param top_n Number of top stations to return (default 15)
#' @return tibble with station details, trip counts, and cumulative share
decompose_topic_stations <- function(uni, topic, top_n = 15) {
  od_enriched %>%
    filter(
    ...
    mutate(
      rank    = row_number(),
      pct     = trips / sum(trips) * 100,
      cum_pct = cumsum(pct)
    ) %>%
    slice_head(n = top_n) %>%
    select(
      rank, DESTINATION_PT_CODE, destination_name, destination_mode,
      trips, pct, cum_pct, dest_lon, dest_lat
    )
}
```

### SMU → Industrial & Automotive (z = 281.66)

```{r smu-industrial-decomposition, message=FALSE}
smu_industrial <- decompose_topic_stations("SMU", "Industrial & Automotive")
...
kable(
  ...
  caption = paste0(
    "Top destinations driving SMU's over-indexing on Industrial & Automotive. ",
    "These ", nrow(smu_industrial), " stations account for ",
    round(max(smu_industrial$cum_pct), 1),
    "% of SMU→Industrial trips."
  )
)
```

### SMU → Education & Professional Services (z = 473.58)

```{r smu-education-decomposition, message=FALSE}
smu_education <- decompose_topic_stations("SMU", "Education & Professional Services")
...
kable(
  ...
  caption = paste0(
    "Top destinations driving SMU's over-indexing on Education & Professional Services. ",
    "These ", nrow(smu_education), " stations account for ",
    round(max(smu_education$cum_pct), 1),
    "% of SMU→Education trips."
  )
)
```

### NUS → Dining & Hospitality (z = 328.73)

```{r nus-dining-decomposition, message=FALSE}
nus_dining <- decompose_topic_stations("NUS", "Dining & Hospitality")
...
kable(
  ...
  caption = paste0(
    "Top destinations driving NUS's over-indexing on Dining & Hospitality. ",
    "These ", nrow(nus_dining), " stations account for ",
    round(max(nus_dining$cum_pct), 1),
    "% of NUS→Dining trips."
  )
)
```

### Station-Level Topic Decomposition Map

Map the top contributing stations for all three key associations on a
single Singapore basemap to visualise their spatial distribution.

```{r station-decomposition-map, message=FALSE, fig.width=10, fig.height=7}
# Combine all three decompositions with labels
topic_assoc_stations <- bind_rows(
  smu_industrial %>% mutate(association = "SMU → Industrial"),
  smu_education   %>% mutate(association = "SMU → Education"),
  nus_dining      %>% mutate(association = "NUS → Dining")
) %>%
  filter(!is.na(dest_lon), !is.na(dest_lat))

assoc_colors <- c(
  "SMU → Industrial"  = "#E15759",
  "SMU → Education"   = "#4E79A7",
  "NUS → Dining"      = "#59A14F"
)

ggplot() +
  geom_sf(data = sg_subzones_proj, fill = "grey92", colour = "white", linewidth = 0.2) +
  geom_point(
    data = topic_assoc_stations,
    aes(x = dest_lon, y = dest_lat, colour = association, size = trips),
    alpha = 0.7
  ) +
  ggrepel::geom_text_repel(
    data = topic_assoc_stations %>%
      group_by(association) %>%
      slice_head(n = 5) %>%  # label top 5 per association
      ungroup(),
    aes(x = dest_lon, y = dest_lat, label = destination_name),
    size = 2.5, max.overlaps = 20, segment.size = 0.3
  ) +
  scale_colour_manual(values = assoc_colors) +
  scale_size_continuous(range = c(1.5, 8), labels = comma) +
  coord_sf(crs = 3414) +
  map_theme +
  labs(
    title = "Top Stations Driving University–Topic Associations",
    subtitle = "Station size proportional to trip volume; labels show top 5 per association",
    colour = "Association",
    size = "Trips"
  )
```
